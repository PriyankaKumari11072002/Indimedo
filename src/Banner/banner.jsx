...
const Banner = () => {
  ...
  useEffect(() => {
    // Add any additional logic you may need on component mount or updates
  }, [bannerCurrIndex]);
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center gap-2 bg-white" style={{ padding: '20px 40px' }}>
        <button onClick={handlePrev} style={{ marginRight: '-9px', zIndex: '0' }}>
          {'<'}
        </button>
        {currentBannerShow.length > 0 &&
          currentBannerShow.map((img) => (
            <div className="" key={img.id}>
              <img
                src={img.banner}
                alt={`image ${img.id}`}
                className=""
                style={{ width: '350px', height: '180px', borderRadius: '10px' }}
                onError={(e) => {
                  e.target.src = 'https://via.placeholder.com/150/cccccc'; // Fallback image URL
                }}
              />
            </div>
          ))}
        <button onClick={handleNext} className="btn-primary" style={{ marginLeft: '-95px', zIndex: '0' }}>
          {'>'}
        </button>
      </div>
